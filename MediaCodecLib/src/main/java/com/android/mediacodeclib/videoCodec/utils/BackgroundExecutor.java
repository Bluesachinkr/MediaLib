...
public class BackgroundExecutor {
    ...
    public static final Executor defaultExecutor = Executors.newScheduledThreadPool(2 * Runtime.getRuntime().availableProcessors());
    ...
    private static Future<?> directExecute(Runnable runnable, long delay) {
        Future<?> future = null;
        if (delay > 0) {
            if ((executor instanceof ScheduledExecutorService) == false) {
                throw new IllegalArgumentException("Executor does not schedulling");
            }
